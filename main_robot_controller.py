...
class MockGoalHandle:
    ...
    async def get_result_async(self):
        # Simulate some processing time for navigation
        await asyncio.sleep(random.uniform(0.5, 1.5)) # Mock nav time
        class MockResult:
            def __init__(self, status):
                self.status = status
        return MockResult(status=self.status) # STATUS_SUCCEEDED

class MockActionClient:
    def __init__(self, node, action_type, action_name):
        self._node = node # In NO ROS, node is the MainRobotController instance
    ...
    async def send_goal_async(self, goal_msg):
        ...
        # Simulate some processing for goal acceptance
...
from automated_benchmarking_llm import AutomatedBenchmarkingLLM # For automated user

# --- Global State ---
current_speaker_role = None 
speech_interface_global = SpeechInterface()
_running_main_loop = True # Initialize at module level

# Need asyncio for the mock action client's async methods
import asyncio

def _yaw_to_quaternion(yaw_radians): # Still useful for potential future use or structure
    q = {} # Simplified for no ROS
    q['x'] = 0.0
    q['y'] = 0.0
...
class MainRobotController: # Changed from Node to object
    def __init__(self):
        self._logger = MockLogger() # Use mock logger
    ...
    async def send_navigation_goal(self, x, y, theta_degrees): 
        ...
        try:
            goal_handle = await self._nav_action_client.send_goal_async(goal_msg_dummy) 
        except Exception as e:
            self.get_logger().error(f"MockNav: Error during send_goal_async: {e}")
            return False

        if not goal_handle or not goal_handle.accepted:
            self.get_logger().error('MockNav: Goal was rejected by the server.')
            return False
        ...
        try:
            ...
            self.get_logger().error(f"MockNav: Error during get_result_async: {e}")
            return False
        
        if result.status == 4: 
            self.get_logger().info('MockNav: Navigation goal succeeded!')
            return True
        else:
            self.get_logger().error(f'MockNav: Navigation goal failed with mock status: {result.status}')
            return False
    ...
    async def _handle_iterative_store_visits_no_ros(self, original_request_parsed_data, ranked_store_options_with_details, shopping_for_user_role, active_profile_for_recs):
        ...
        for i, store_option_details in enumerate(ranked_store_options_with_details):
            ...
            if map_x is None or map_y is None:
                self.speech.say(f"Sorry, I don't have precise location data to simulate navigation to {store_name}.")
                nav_success = False
            else:
                nav_success = await self.send_navigation_goal(map_x, map_y, map_theta) 
            ...
            if nav_success:
                ...
                confirm_context_next_store = {"item_summary": request_summary} # Generic context
                if not is_last_option_in_list and self.get_confirmation_dialogue("Try the next store suggestion?", 
                                                                                dialogue_state_hint_for_auto_llm="confirm_visit_store", # Re-use hint, context is less specific
            ...
            next_action_context = {
                ...
                "was_last_item_fulfilled": any_request_fulfilled_overall,
                "has_more_options": has_more_options
            }
            next_action_response = self.speech.listen_and_get_text(
                dialogue_state_hint="decide_next_action",
                context_for_auto_llm=next_action_context
            )
    ...
    async def process_intent_no_ros(self, intent_from_llm, parsed_llm_data, shopping_for_user_role, active_profile_for_recs): 
        ...
        if intent_from_llm in {"find_product", "find_store"}:
            ...
            if current_pose_tuple == (None, None, None): 
                self.speech.say("Warning: My current location is unclear (mock), so recommendations might not be distance-optimized.")
            ...
            return await self._handle_iterative_store_visits_no_ros( 
    ...
    async def run_shopping_session_loop(self): 
        global current_speaker_role, _running_main_loop # _running_main_loop already global
        ...
        while _running_main_loop: 
            if not current_user_input or current_user_input.strip().lower() == "stop":
                ...
                parsed_llm_output = self.llm_manager.generate_structured_query(
                    ...
                    shopping_for_user_profile_context=self.profile_manager.get_profile(current_speaker_role) 
                )
            ...
            if session_status.get("status") == "new_request":
                ...
                # _running_main_loop = False; # This will be set in main_async_runner's finally block too
                break 
            else: 
...
async def main_async_runner(args=None): 
    # Ensure _running_main_loop is global here if modified in finally
    global _running_main_loop, current_speaker_role
    _running_main_loop = True # Set before use
    current_speaker_role = None # Reset for each run
    ...
    if USE_AUTOMATED_LLM_USER:
        ...
        print(f"\n<<<<< RUNNING IN AUTOMATED BENCHMARKING MODE (Persona: {AUTOMATED_USER_PERSONA}) >>>>>\n")
    else:
        pass
    
    try:
        await main_controller.run_shopping_session_loop() 
    except KeyboardInterrupt:
        main_controller.get_logger().info("KeyboardInterrupt received, shutting down.")
    except Exception as e:
        main_controller.get_logger().error(f"Unhandled exception: {e}", exc_info=True)
    finally:
        _running_main_loop = False # Ensure loop terminates if exception occurred
        main_controller.get_logger().info("Shutting down MainRobotController (NO ROS)...")
